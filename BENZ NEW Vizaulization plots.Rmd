...
```{r}
 ---
#title: 'MercEDAs - visualising categorical features'
...
train %>%select(X4) %>% group_by(X4) %>%summarise(count = n())
```
...
```{r}
dummy <- train %>%select(ID,y,X10:X385)
...
train %>%select(X10:X385) %>%mutate_at(vars(starts_with("X")), funs(as.integer)) %>%prcomp() %>%
